...
import { moveFromDirty } from "./functions/function";
...
export const RoomCard = ({ item }) => {
  return (
    <div className="bg-gray-200 w-44 h-44 rounded-2xl flex flex-col p-4">
      ...
      <div className="text-base text-center my-auto">{item.roomNumber}</div>
      <div className="flex">
        <div
        ...
        <button
          ...
          onClick={() => moveFromDirty(item.roomNumber)}
        >
          <IoCheckbox size={28} color="#31A65C" />
        </button>
      </div>
    </div>
  );
